...
function StreamingS3(stream, s3AccessKey, s3SecretKey, s3Params, options, cb) {
  ...
  if (stream) {
    ...
    this.emit('error', new Error('You must provide a readable stream.'));
  }
...
StreamingS3.prototype.sendToS3 = function() {
  ...
  function uploadChunk(chunk, next) {
    ...
    // If we have a valid writeable stream to pipe to as well, write the chunk
    if (self.pipeto && self.pipeto instanceof nodestream.Writeable) {
      self.pipeto.write(chunk);
    }
